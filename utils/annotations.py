...
def download_gtf_if_needed(
    ...
    *,
    ...
    if not allow_download:
        raise FileNotFoundError(f"Missing cached GTF file: {gtf_path}")
...
def load_gene_annotations_for_gene(
    ...
    *,
    ...
    genome_prefix = _gtf_cache_prefix(gtf_path)
    cache_candidates = [
        (
            shortcut_dir / f"{genome_prefix}_{gene_name}_gene_annotations.csv",
            shortcut_dir / f"{genome_prefix}_{gene_name}_exon_annotations.csv",
        ),
        (
            shortcut_dir / f"{gene_name}_gene_annotations.csv",
            shortcut_dir / f"{gene_name}_exon_annotations.csv",
        ),
    ]

    if shortcut_dir.exists():
        for gene_file, exon_file in cache_candidates:
    ...
    if write_shortcuts:
        ensure_dir(shortcut_dir)
        gene_file, exon_file = cache_candidates[0]
...
def _gtf_cache_prefix(gtf_path: Path) -> str:
    name = Path(gtf_path).name
    if name.endswith(".gtf.gz"):
        return name[: -len(".gtf.gz")]
    if name.endswith(".gtf"):
        return name[: -len(".gtf")]
    return Path(gtf_path).stem
